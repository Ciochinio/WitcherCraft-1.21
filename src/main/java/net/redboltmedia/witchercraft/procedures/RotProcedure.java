	...
	public static void execute(LevelAccessor world, Entity entity) {
		...
		if (!world.getEntitiesOfClass(Mob.class, new AABB(Vec3.ZERO, Vec3.ZERO)
			...
			if (entity instanceof Player _player && !_player.level().isClientSide())
				_player.displayClientMessage(Component.literal(("cwel  detected" + new java.text.DecimalFormat("##.##").format(entity.getData(WitchercraftModVariables.PLAYER_VARIABLES).test))), false);
		} else {
			{
				WitchercraftModVariables.PlayerVariables _vars = entity.getData(WitchercraftModVariables.PLAYER_VARIABLES);
				_vars.test = 0;
